...
namespace CSharp
{
    ...
    {
        // Api 객체선언
        public static DBCommAgentApi api = null!;
        ...
        public async Task Main()
        {
            ...
            // 로그인
            if (!api.Login(Secret.UserId, Secret.Password, Secret.CertPwd))
            {
                print($"연결실패: {api.LastMessage}");
                return;
            }
            print($"연결성공, 접속서버: {(api.IsSimulation ? "모의투자" : "실투자")}");
        ...
        public static void print(OutRecData? data)
        {
            if (data == null) return;

            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"{data.RecName}, Row Count = {data.OutDatas.Length}, Field Count = {data.FieldInfos.Count}");
            Console.ForegroundColor = ConsoleColor.Gray;

            var table = new ConsoleTable();

            int nColumCount = data.FieldInfos.Count;
            if (nColumCount > 0)
            {
                var columNames = data.FieldInfos.Select(x => x.CAPTION);
                table.AddColumn(columNames);
            }
            else
            {
                var columNames = Enumerable.Range(1, data.OutDatas[0].Length).Select(x => x.ToString());
                table.AddColumn(columNames);
            }
            foreach (var row in data.OutDatas)
            {
                table.AddRow(row);
            }
